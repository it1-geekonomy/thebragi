  ...
  return (
    <section className="bg-black px-5 py-16 sm:px-8 lg:px-10">
      <SectionHeading eyebrow="Plans" title="Start with the plan that matches your motion">
        Pricing is shown in INR. Billed monthly. Terms apply.
      </SectionHeading>
      <div className={`mx-auto mt-10 grid max-w-6xl gap-5 ${plans.length >= 3 ? 'lg:grid-cols-3' : plans.length === 2 ? 'lg:grid-cols-2' : 'lg:grid-cols-1 max-w-lg'}`}>
        {isLoading ? (
          [1, 2, 3].map((i) => (
          ...
          plans.map((plan) => (
            <Card key={plan.slug} className={plan.popular ? "border-[#7dc890]/50 p-6" : "p-6"}>
              ...
              {activePlan === plan.slug ? (
                <Link className="mt-6 inline-flex w-full items-center justify-center rounded-md bg-white/10 px-4 py-3 text-sm font-semibold text-white cursor-default pointer-events-none" href={ROUTES.dashboard}>Current plan</Link>
              ) : (
