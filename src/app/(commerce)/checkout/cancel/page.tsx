...
export default async function CheckoutCancelPage({
  ...
  const checkoutHref = ROUTES.checkout(plan, {
    users: params.users ? Number(params.users) : undefined,
  ...
  return (
    <main className="mx-auto max-w-3xl px-5 py-16 text-center sm:px-8">
      ...
      <h1 className="mt-5 text-4xl font-semibold text-white sm:text-5xl">
        {params.mode === "trial" ? "Trial not started" : "Payment cancelled"}
      </h1>
      <p className="mx-auto mt-4 max-w-xl text-base leading-8 text-white/58">
        {params.mode === "trial"
          ? "No INR 1 authorization was captured. Your account and plan selection are saved — return to checkout when you are ready, or pick a different plan."
          : "No payment was captured. Your account and plan selection are saved — return to checkout to buy the plan, or pick a different plan."}
      </p>
      <div className="mt-8 flex flex-wrap justify-center gap-3">
        <Link className="rounded-md bg-[#5f9965] px-5 py-3 text-sm font-semibold text-white" href={checkoutHref}>
          {params.mode === "trial" ? "Return to trial checkout" : "Return to checkout"}
        </Link>
        <Link
